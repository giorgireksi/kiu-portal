...
| **Bare portal pages** (22) | shared paint + layout-only `lux-page-bare-lite` + `lux-full-paint` |
...
|-------------|----------------|-------|
| Panel / soft-chrome glass | `lux-tokens.css` (§3 paint) | `--lux-panel-*`, `--lux-soft-chrome-*` |
...
**Samples:** bare stack = `students-admin.html` (modals + bare-lite, no layout-portal link); dashboard = `index.html` (fouc-ht + index-home-*).
...
Catalog ceiling **≤ 60** (`phase-a-css-stack-guard`). Shared paint budget for bare portals **≤ 3000** canonical lines (`bare-css-budget`). Live `assets/css` (ex-`_archive`) **≤ 6500** lines: nested readable chrome (`lux-shell` / `layout-portal`); complex rules stay multi-line. Day-1 map: [`css-handoff.md`](css-handoff.md).
...
Optional: promote FOUC atmosphere to bare portals; merge `lux-focus-panel` into shell if desired.
