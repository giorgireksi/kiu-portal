...
| id | UI label | Owner |
|----|----------|--------|
| `feed` | Home | social-feed.js (hero + panel + compose/attach + `renderFeedOwnedDialog` + `handleSocialFeedClick` for `post-*`/`comment-*`/`story-*`/`focus-feed`/`feed-refresh`); page dispatches via `isSocialFeedClickAction` |
