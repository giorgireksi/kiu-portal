...
# The clean HTML to inject (single copy)
$cleanNav = @'
...
function openSheet(){var s=document.getElementById('mobile-action-sheet');if(!s)return;s.style.display='';document.body.style.overflow='hidden';requestAnimationFrame(function(){s.classList.add('is-open')});updAdmin()}
...
function updAdmin(){var a=document.getElementById('mob-sheet-admin-section');if(!a)return;var r='';try{r=typeof getEffectiveRole==='function'?getEffectiveRole():(typeof getEffectiveUserRole==='function'?getEffectiveUserRole():'')}catch(e){}a.style.display=(r==='admin'||r==='student_service')?'':'none'}
...
function init(){autoCollapse();setupNav();setupSheet();onResize();var ht=setInterval(function(){if(typeof window.navigate==='function'){clearInterval(ht);hookNav();updAdmin()}},200);setTimeout(function(){clearInterval(ht)},10000);window.addEventListener('resize',onResize);
...
foreach ($file in $htmlFiles) {
    ...
    # STEP 1: Strip ALL old mobile nav / action sheet / JS blocks
    # Remove all <!-- MOBILE BOTTOM NAVIGATION ... </nav> blocks
    while ($content -match '(?s)<!--\s*MOBILE BOTTOM NAVIGATION.*?</nav>') {
        $content = $content -replace '(?s)<!--\s*MOBILE BOTTOM NAVIGATION.*?</nav>\s*', ''
    }

    # Remove all <!-- MOBILE ACTION SHEET ... closing divs
    while ($content -match '(?s)<!--\s*MOBILE ACTION SHEET.*?mob-sheet-close.*?</div>\s*</div>\s*</div>') {
        $content = $content -replace '(?s)<!--\s*MOBILE ACTION SHEET.*?mob-sheet-close.*?</div>\s*</div>\s*</div>\s*', ''
    }

    # Remove all <!-- MOBILE EXPERIENCE CONTROLLER ... </script>
    while ($content -match '(?s)<!--\s*MOBILE EXPERIENCE CONTROLLER.*?</script>') {
        $content = $content -replace '(?s)<!--\s*MOBILE EXPERIENCE CONTROLLER.*?</script>\s*', ''
    }

    # STEP 2: Clean up any leftover empty lines
    $content = $content -replace '(\r?\n){4,}', "`n`n"

    # STEP 3: Insert clean copy before </body>
    $content = $content -replace '</body>', "$cleanNav`n</body>"
