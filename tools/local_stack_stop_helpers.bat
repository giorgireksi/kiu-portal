...
set /p PID=<"%pid_file%"
if defined PID (
    tasklist /FI "PID eq %PID%" /NH 2>nul | findstr /B /C:"%PID%" >nul 2>&1
    if not errorlevel 1 (
...
set "KILL_PATTERN=%~2"
set "KILL_ROOT=%~3"
powershell -NoProfile -Command "$pattern='%KILL_PATTERN%'; $pattern = $pattern -replace '\\','/'; Get-CimInstance Win32_Process -ErrorAction SilentlyContinue | Where-Object { $_.Name -match '^(node|electron)\.exe$' -and $_.CommandLine -and (($_.CommandLine -replace '\\','/') -like ('*' + $pattern + '*')) } | ForEach-Object { try { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue } catch {} }" >nul 2>&1
set "KILL_PATTERN="
set "KILL_ROOT="
exit /b 0
