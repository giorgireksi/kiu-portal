...
# Role-aware action sheet + updated JS with 1024px breakpoint
$cleanBlock = @'
...
<div id="mobile-action-sheet" class="mob-sheet" style="display:none;" role="dialog" aria-modal="true">
    ...
    <div class="mob-sheet-panel">
        ...
        <!-- Admin Pages (only shown for admin role) -->
        ...
        <!-- Student Service Pages (only for student_service role) -->
...
if(xb)xb.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();toggleSheet()});
}
...
function setupSheet(){
var bd=document.getElementById('mob-sheet-backdrop');if(bd)bd.addEventListener('click',closeSheet);
var cb=document.getElementById('mob-sheet-close');if(cb)cb.addEventListener('click',closeSheet);
var sh=document.getElementById('mobile-action-sheet');if(!sh)return;
sh.querySelectorAll('.mob-sheet-nav-btn[data-nav-target]').forEach(function(b){b.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();var t=b.getAttribute('data-nav-target');if(typeof window.navigate==='function')window.navigate(t);syncActive(t);closeSheet();closeSB()})});
var as=document.getElementById('mob-act-admin');if(as)as.addEventListener('click',function(e){e.preventDefault();closeSheet();var p=document.querySelector('.lux-picker-btn');if(p){p.click();return}var v=document.querySelector('#admin-view-select,[name="admin-view"]');if(v)v.click()});
var ts=document.getElementById('mob-act-theme');if(ts)ts.addEventListener('click',function(e){e.preventDefault();closeSheet();openStd()});
var ps=document.getElementById('mob-act-profile');if(ps)ps.addEventListener('click',function(e){e.preventDefault();closeSheet();if(typeof window.navigate==='function')window.navigate('profile-view')});
var ls=document.getElementById('mob-act-lightmode');if(ls)ls.addEventListener('click',function(e){e.preventDefault();document.body.classList.toggle('lux-light-mode');var il=document.body.classList.contains('lux-light-mode');ls.querySelector('span').textContent=il?'Dark Mode':'Light Mode';ls.querySelector('i').className=il?'fas fa-moon':'fas fa-sun';localStorage.setItem('kiuLuxuryLightMode',il?'1':'0')});
}
...
function updRoleNav(){
var role=getRole();
var adminSec=document.getElementById('mob-sheet-admin-section');
var serviceSec=document.getElementById('mob-sheet-service-section');
if(adminSec)adminSec.style.display=(role==='admin')?'':'none';
if(serviceSec)serviceSec.style.display=(role==='admin'||role==='student_service')?'':'none';
var adminSwitch=document.getElementById('mob-act-admin');
if(adminSwitch)adminSwitch.style.display=(role==='admin'||role==='student_service')?'':'none';
}
...
function onResize(){var n=document.getElementById('mobile-bottom-nav');if(!n)return;n.style.display=isMob()?'':'none';if(!isMob()){closeSheet();var s=document.getElementById('mobile-action-sheet');if(s)s.style.display='none'}}
function init(){
autoCollapse();setupNav();setupSheet();onResize();
var ht=setInterval(function(){if(typeof window.navigate==='function'){clearInterval(ht);hookNav();updRoleNav()}},200);
setTimeout(function(){clearInterval(ht)},10000);
...
foreach ($file in $htmlFiles) {
    ...
    # Remove ALL existing mobile blocks
    $i = 0
    while ($i -lt $lines.Count) {
        $line = $lines[$i]

        # Start of a mobile block?
        if ($line -match '<nav id="mobile-bottom-nav"' -or
            $line -match '<div id="mobile-action-sheet"' -or
            ($line -match '<script>' -and $i + 1 -lt $lines.Count -and $lines[$i+1] -match 'initMobileExperience')) {
            $inMobileBlock = $true
        }

        if ($inMobileBlock) {
            # Find end of block
            if ($line -match '</nav>' -or $line -match '</script>') {
                # Check: if this is the action sheet div, we need to find all closing divs
                if ($line -match '</nav>' -or $line -match '</script>') {
                    $inMobileBlock = $false
                    $i++
                    continue
            ...
            # For action sheet: it ends with </div> stack
            if ($line -match 'mob-sheet-close') {
                # Skip forward to find all closing </div>s
                while ($i -lt $lines.Count -and $lines[$i] -notmatch '</div>\s*$' -or $lines[$i] -match 'mob-sheet') {
                    $i++
                }
                # Skip remaining closing divs
                while ($i -lt $lines.Count -and $lines[$i].Trim() -match '^</div>$') {
                    $i++
                }
                $inMobileBlock = $false
                continue
    ...
    foreach ($ln in $newLines) {
        if ($ln -match '</body>') {
            foreach ($bl in $cleanBlock -split "`n") {
                $finalLines.Add($bl)
            }
        }
        $finalLines.Add($ln)
    ...
    [IO.File]::WriteAllLines($file.FullName, $finalLines.ToArray())
    Write-Host "  Updated: $($file.Name)"
}

Write-Host "`nDone. All pages updated with role-aware nav + 1024px breakpoint."
