...
describe('exams route regressions.test', () => {
    it('bare shell: no luxury/surfaces paint sheets; flat bare CSS', () => {
        ...
        expect(bare).toContain('body.lux-route-exams #admin-exams-root .ex2-builder-toolbar');
        expect(bare).toContain('body.lux-route-exams #admin-exams-root .ex2-builder-summary-strip');
        expect(bare).toContain('body.lux-route-exams #admin-exams-root .ex2-summary-chip-label');
        expect(bare).toContain('.ex2-modal-search.lux-control');
        expect(bare).toContain('body.lux-route-exams #admin-exams-root .ex2-modal-list .ex2-list-item');
        expect(bare).toContain('body.lux-route-exams #admin-exams-root .ex2-btn:disabled');
        expect(bare).toContain(':is(.ex2-input, .ex2-textarea, .ex2-select):not(.lux-control):hover:not(:disabled)');
        expect(bare).toContain('body.lux-route-exams #admin-exams-root .ex2-field--picker .lux-picker-btn--compact');
        expect(bare).toContain('.ex2-btn.is-secondary:not(.lux-secondary-btn):hover');
        ...
        expect(fouc).toContain('.ex2-modal-list .ex2-list-item');
        ...
        expect(builder).toContain('ex2-builder-toolbar lux-soft-chrome');
        expect(builder).toContain('lux-primary-btn');
        expect(builder).toContain('lux-control lms-route-input');
        expect(builder).toContain('ex2-question-bank');
        expect(builder).toContain('ex2-variant-workspace');
        expect(builder).toContain('class="ex2-qnav-label"');
        expect(builder).toContain('ex2-variant-hint lms-route-meta-12');
        expect(builder).not.toContain('ex2-qnav-label lms-route-field-label');
        expect(builder).not.toMatch(/exam-auto-gen-count" class="ex2-input/);
        expect(bare).toContain('#admin-exams-root .ex2-qnav-bar {');
        expect(bare).toContain('#admin-exams-root .ex2-option {');
        expect(bare).toContain('#admin-exams-root .ex2-variant-workspace {');
        expect(bare).toContain('#admin-exams-root .ex2-auto-gen-box.lux-soft-chrome {');
        ...
        expect(consoleJs).toContain('ex2-modal-meta lms-route-meta-12');
        expect(consoleJs).not.toMatch(/class="ex2-input ex2-modal-search"/);
