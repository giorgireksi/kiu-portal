...
describe('Student Service split workspace regressions', () => {
    ...
    it('loads the split-workspace student-service bundle through a real standalone entry', () => {
        ...
        expect(studentServiceHtml).toContain('assets/js/features/navigation.js?v=20260817-instantassembly1');
...
describe('Q&A comment reply guardrails', () => {
    ...
    it('delegates QA thread clicks from the modal root host', () => {
        ...
        expect(source).toContain('[data-student-service-owner-resolution-cycle]');
        expect(source).toContain('getStudentServiceOwnerResolutionCycleTarget');
    ...
    it('keeps inline reply and open-thread state out of QA feed cache keys', () => {
        ...
        const renderPageBlock = extractStudentServiceFnBlock(source, 'renderStudentServicePage');
        ...
        expect(questionFeedbackFn).toContain('question: questionFromPayload');
        expect(questionFeedbackFn).toContain("ariaPressed === 'true'");
        expect(questionFeedbackFn).not.toContain('payloadVoted !== expectedVoted');
        expect(questionFeedbackFn).not.toMatch(/patchStudentServiceOpenQuestionThread/);
        const ownerResolutionFn = extractStudentServiceFnBlock(staff, 'setStudentServiceQuestionOwnerResolution');
        expect(ownerResolutionFn).not.toContain('aria-pressed');
        expect(ownerResolutionFn).not.toMatch(/patchStudentServiceOpenQuestionThread/);
        expect(ownerResolutionFn).not.toMatch(/flashStudentServiceActionButton\(triggerButton, 'success'\)/);
        expect(source).toContain('options.question || getStudentServiceQuestionById(questionId)');
        expect(source).toContain('function patchStudentServiceAnswerHelpfulBtn(');
        expect(source).toContain('options.answer || findStudentServiceAnswerRecord(question, answerId)');
        expect(source).toContain('article.social-neo-comment[data-student-service-answer-id=');
        const answerFeedbackBlock = extractStudentServiceFnBlock(staff, 'setStudentServiceAnswerFeedback');
        expect(answerFeedbackBlock).toContain('patchStudentServiceAnswerHelpfulBtn(normalizedQuestionId, normalizedAnswerId, {');
        expect(answerFeedbackBlock).toContain('answer: answerFromPayload');
        expect(answerFeedbackBlock).not.toContain('patchStudentServiceOpenQuestionThread');
        expect(answerFeedbackBlock).not.toContain('flashStudentServiceActionButton(triggerButton, \'success\')');
        expect(answerFeedbackBlock).not.toContain('refreshStudentServiceDataAndRender');
        expect(answerFeedbackBlock).not.toContain('studentServiceHelpfulPending');
        expect(answerFeedbackBlock).toContain('pendingAnswerHelpfulIds');
        expect(answerFeedbackBlock).toContain('buildStudentServiceAnswerHelpfulToggleSnapshot');
        expect(answerFeedbackBlock).toContain('{ helpful: !wasHelpful }');
        ...
        expect(answerFeedbackBlock).toContain('findStudentServiceAnswerRecord(q, normalizedAnswerId)');
        ...
        expect(source).toContain('flashStudentServiceActionButton(triggerButton, \'error\')');
    ...
    it('exposes owner resolution controls for question authors and public status labels', () => {
        ...
        expect(source).toContain('data-student-service-owner-resolution-cycle="true"');
        expect(source).toContain('getStudentServiceOwnerResolutionCycleTarget');
