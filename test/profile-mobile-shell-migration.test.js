...
function bootProfileMobileShell(options = {}) {
  const dom = new JSDOM(
    `<!DOCTYPE html><html><head></head><body class="lux-route-profile">
      <button id="lux-sidebar-toggle"></button>
      <nav id="mobile-bottom-nav" class="profile-mobile-hidden" aria-label="Mobile navigation" style="display:none;">
      ...
      <div id="mobile-action-sheet" class="profile-mobile-hidden" style="display:none;">
        <div id="mob-sheet-backdrop"></div>
        <div id="mob-sheet-dynamic-nav"></div>
        <button id="mob-sheet-close" type="button" data-modal-close="1"></button>
    ...
    {
      url: 'http://localhost/profile.html?view=student',
...
describe('profile mobile shell migration', () => {
  it('migrates the page onto the shared standalone mobile shell contract', () => {
    const source = readSource('profile.html');
    ...
    expect(source).toContain('<nav id="mobile-bottom-nav"');
    expect(source).toContain('class="mob-sheet"');

    expect(classificationModuleSource).toContain("'profile.html': { category: 'standard-shell', dedicatedCss: ['assets/css/profile-route.css'], mobileShell: 'shared-standalone' }");
    expect(classificationSource).toContain("| `profile.html` | `standard-shell` |");
  });

  it('uses the shared standalone shell on first tap and clears the default home active state', () => {
    const dom = bootProfileMobileShell({ withNavigate: false });
