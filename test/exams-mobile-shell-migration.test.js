...
function bootExamsMobileShell(options = {}) {
  const dom = new JSDOM(
    `<!DOCTYPE html><html><head></head><body class="lux-route-exams">
    ...
    {
      url: 'http://localhost/exams.html?view=professor',
  ...
  dom.window.__KIU_STANDALONE_MOBILE_SHELL_CONFIG = {
    ...
    navByRole: {
      professor: [
        {
          group: 'Faculty',
          items: [['home', 'Dashboard', 'fas fa-th-large'], ['news', 'News', 'fas fa-newspaper'], ['exams', 'Exams', 'fas fa-file-signature'], ['timetable', 'Schedule', 'fas fa-calendar-week']]
        }
      ]
    }
  };
...
describe('exams mobile shell migration', () => {
  it('migrates the page onto the shared standalone mobile shell contract', () => {
    ...
    expect(source).toContain("activeTarget: 'exams'");
  ...
  it('uses the shared standalone shell on first tap and clears the default home active state', () => {
    const dom = bootExamsMobileShell({ withNavigate: false });
    ...
    doc.querySelector('#mob-sheet-dynamic-nav [data-nav-target="timetable"]')
      ?.dispatchEvent(new dom.window.MouseEvent('click', { bubbles: true }));

    expect(dom.window.resolvePortalRouteUrl).toHaveBeenCalledWith('timetable', 'professor');
    expect(dom.window.location.hash).toBe('#professor-timetable');
  });
});
