...
function bootChancelleryMobileShell(options = {}) {
  const dom = new JSDOM(
    `<!DOCTYPE html><html><head></head><body class="lux-route-chancellery">
    ...
    {
      url: 'http://localhost/chancellery.html?view=student',
  ...
  dom.window.__KIU_STANDALONE_MOBILE_SHELL_CONFIG = {
    activeTarget: 'chancellery',
    adminVisibleRoles: ['admin', 'student_service', 'professor', 'ta'],
    navByRole: {
      student: [
        {
          group: 'Support',
          items: [['chancellery', 'E-Chancellery', 'fas fa-desktop'], ['student-service', 'Student Service', 'fas fa-headset'], ['library', 'Library', 'fas fa-book']]
        }
      ]
    }
  };
...
describe('chancellery mobile shell migration', () => {
  it('migrates the page onto the shared standalone mobile shell contract', () => {
    const source = readSource('chancellery.html');
    ...
    expect(source).toContain("activeTarget: 'chancellery'");
    ...
    expect(classificationModuleSource).toContain("'chancellery.html': { category: 'standard-shell', dedicatedCss: ['assets/css/chancellery-route.css'], mobileShell: 'shared-standalone' }");
    expect(classificationSource).toContain("| `chancellery.html` | `standard-shell` |");
  });

  it('uses the shared standalone shell on first tap and clears the default home active state', () => {
    const dom = bootChancelleryMobileShell({ withNavigate: false });
    ...
    expect(dom.window.resolvePortalRouteUrl).toHaveBeenCalledWith('library', 'student');
    expect(dom.window.location.hash).toBe('#student-library');
  });
});
