...
function bootAdminLibraryMobileShell(options = {}) {
  const dom = new JSDOM(
    `<!DOCTYPE html><html><head></head><body class="lux-route-admin-library role-admin">
    ...
    {
      url: 'http://localhost/admin-library.html?view=admin',
...
describe('admin library mobile shell migration', () => {
  ...
  it('uses the shared standalone shell on first tap and clears the default home active state', () => {
    const dom = bootAdminLibraryMobileShell({ withNavigate: false });
    ...
    doc.querySelector('#mob-sheet-dynamic-nav [data-nav-target="orders"]')
      ?.dispatchEvent(new dom.window.MouseEvent('click', { bubbles: true }));

    expect(dom.window.resolvePortalRouteUrl).toHaveBeenCalledWith('orders', 'admin');
    expect(dom.window.location.hash).toBe('#admin-orders');
  });
});
