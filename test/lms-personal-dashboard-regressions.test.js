...
describe('LMS personal dashboard regressions', () => {
    ...
    it('exposes unified history API and autosave hooks on the frontend', () => {
        ...
        expect(runtime).toContain('open-history-panel');
        expect(runtime).toContain('close-history-panel');
    ...
    it('overlay uses shared modal shell, portal paint, and typography', () => {
        ...
        expect(modals).toContain('.lms-personal-dashboard-card');
        expect(fouc).toMatch(/#lms-personal-dashboard-overlay \.lms-personal-dashboard-card\s*\{[\s\S]*?--lux-popup-shell-surface/);
        expect(fouc).toMatch(/#lms-personal-dashboard-overlay \.lms-personal-dashboard-card\s*\{[\s\S]*?--lux-warmglass-surface/);
        expect(fouc).not.toMatch(/#lms-personal-dashboard-overlay \.lms-personal-dashboard-card\s*\{[^}]*--lux-panel-fill/);
        expect(fouc).not.toContain('#lms-personal-dashboard-overlay .lms-personal-dashboard-card {\n  background-color: var(--lux-panel-fill)');
        expect(fouc).toContain('#lms-personal-dashboard-overlay .lms-personal-dashboard-share-panel.lux-soft-chrome');
        expect(fouc).toContain('.lms-personal-dashboard-save-name-panel.lux-soft-chrome');
        expect(fouc).toContain('var(--lux-warmglass-overlay-dark)');
        expect(fouc).not.toMatch(/#lms-personal-dashboard-overlay \.lms-personal-dashboard-share-overlay\s*\{[^}]*--lux-panel-fill/);
        expect(fouc).toMatch(/#lms-personal-dashboard-overlay :is\(\s*\n\s*\.lms-personal-dashboard-share-panel\.lux-soft-chrome/);
        expect(fouc).toContain('.lms-personal-dashboard-history-empty.lux-empty-state');
        expect(fouc).toContain('.lms-personal-dashboard-history-panel.lux-soft-chrome');
        expect(fouc).toMatch(/\.lms-personal-dashboard-board-host :is\([\s\S]*?\.lms-whiteboard-tools/);
        expect(fouc).toContain('--wb-hud-border');
        expect(fouc).not.toMatch(/#lms-personal-dashboard-overlay :is\([\s\S]*?\.lms-whiteboard-shell[\s\S]*?\)\s*\{/);
        expect(runtime).toContain("setAttribute('data-lux-transparency-exempt', '1')");
        expect(runtime).toContain('lms-personal-dashboard-head lux-glass-dialog-head');
        expect(runtime).toContain('lms-personal-dashboard-history-panel lux-soft-chrome home-hover-chip');
        expect(runtime).toContain('lms-personal-dashboard-save-name-panel lux-soft-chrome home-hover-chip');
        expect(runtime).toContain('data-lms-personal-dashboard-save-name-overlay');
        expect(runtime).toContain('confirm-save-name');
        expect(runtime).toContain('close-save-name-panel');
        expect(runtime).toContain('openLmsPersonalDashboardSaveNamePanel');
        expect(runtime).not.toContain("window.prompt('Name this saved progress'");
        expect(bare).toContain('.lms-personal-dashboard-layout.is-student-layout');
        expect(bare).toMatch(/\.lms-personal-dashboard-layout\.is-student-layout\s*\{[^}]*grid-template-columns:\s*minmax\(0,\s*1fr\)\s*;/);
        expect(bare).not.toMatch(/\.lms-personal-dashboard-layout\.is-student-layout\s*\{[^}]*minmax\(260px,\s*320px\)/);
        expect(bare).toMatch(/#lms-personal-dashboard-overlay\.lms-glass-dialog-overlay\s*\{[^}]*padding:\s*0/);
        expect(bare).toMatch(/#lms-personal-dashboard-overlay \.lms-personal-dashboard-card\s*\{[\s\S]*?height:\s*100%/);
        expect(bare).not.toMatch(/#lms-personal-dashboard-overlay \.lms-personal-dashboard-card\s*\{[^}]*min\(90vh,\s*920px\)/);
        expect(bare).toContain('#lms-personal-dashboard-overlay');
        expect(bare).toMatch(/#lms-personal-dashboard-overlay[\s\S]*?\.lms-whiteboard-shell--personal\.is-props-open \.lms-whiteboard-props/);
        expect(bare).toMatch(/#lms-personal-dashboard-overlay[\s\S]*?\.lms-whiteboard-shell--personal \.lms-whiteboard-props\s*\{[\s\S]*?width:\s*0/);
        expect(bare).toMatch(/#lms-personal-dashboard-overlay[\s\S]*?\.lms-whiteboard-layout--personal\s*\{[\s\S]*?grid-template-rows:\s*minmax\(0,\s*1fr\)/);
        expect(bare).toMatch(/#lms-personal-dashboard-overlay[\s\S]*?\.lms-whiteboard-panel\.lms-live-panel\s*\{[\s\S]*?overflow:\s*hidden/);
        expect(bare).toMatch(/#lms-personal-dashboard-overlay \.lms-whiteboard-minimap-shell\s*\{[^}]*flex-direction:\s*column-reverse/);
        ...
        expect(bare).toContain('.lms-personal-dashboard-share-overlay[hidden]');
        expect(bare).toContain('.lms-personal-dashboard-history-overlay[hidden]');
        expect(bare).toContain('.lms-personal-dashboard-save-name-overlay[hidden]');
        expect(bare).toMatch(/\.lms-personal-dashboard-share-overlay[\s\S]*?z-index:\s*60/);
        expect(bare).toMatch(/\.lms-personal-dashboard-history-overlay[\s\S]*?z-index:\s*60/);
        expect(bare).toMatch(/\.lms-personal-dashboard-save-name-overlay[\s\S]*?z-index:\s*61/);
        expect(bare).toContain('.lms-personal-dashboard-share-close');
        expect(bare).toContain('.lms-personal-dashboard-history-close');
        expect(bare).toContain('.lms-personal-dashboard-save-name-close');
        expect(bare).toMatch(/\.lms-whiteboard-shell\[data-lms-whiteboard-theme="light"\] \.lms-wb-input/);
        expect(bare).toMatch(/--lux-warmglass-input/);
        expect(runtime).toContain('close-share-panel');
        expect(runtime).toContain('closeLmsPersonalDashboardSharePanel');
        expect(runtime).toContain('closeLmsPersonalDashboardHistoryPanel');
        expect(runtime).toContain('closeLmsPersonalDashboardSaveNamePanel');
        expect(primitives).toContain('#lms-personal-dashboard-overlay #lms-personal-dashboard-share-title.lux-page-title');
        expect(primitives).toContain('#lms-personal-dashboard-overlay #lms-personal-dashboard-history-title.lux-page-title');
        expect(primitives).toContain('#lms-personal-dashboard-overlay #lms-personal-dashboard-save-name-title.lux-page-title');
        ...
        expect(classroom).toContain('assets/js/pages/lms-personal-dashboard-runtime.js?v=20260731-pdsavename1');
    });
});