...
function bootFacultyGradebookMobileShell(options = {}) {
  const dom = new JSDOM(
    `<!DOCTYPE html><html><head></head><body class="lux-route-faculty-gradebook">
    ...
    {
      url: 'http://localhost/faculty-gradebook.html?view=professor',
  ...
  dom.window.__KIU_STANDALONE_MOBILE_SHELL_CONFIG = {
    activeTarget: 'faculty-gradebook',
    adminVisibleRoles: ['admin', 'student_service', 'professor', 'ta'],
    navByRole: {
      professor: [
        {
          group: 'Faculty',
          items: [['home', 'Dashboard', 'fas fa-th-large'], ['faculty-gradebook', 'Gradebook', 'fas fa-chart-bar'], ['lms', 'LMS', 'fas fa-book-reader']]
        }
      ]
    }
  };
