...
function bootAdminSchedulerMobileShell(options = {}) {
  const dom = new JSDOM(
    `<!DOCTYPE html><html><head></head><body class="lux-route-admin-scheduler role-admin">
    ...
    {
      url: 'http://localhost/admin-scheduler.html?view=admin',
...
describe('admin scheduler mobile shell migration', () => {
  it('migrates the page onto the shared standalone mobile shell contract', () => {
    const source = readSource('admin-scheduler.html');
    ...
    expect(source).toContain("activeTarget: 'admin-scheduler'");
  ...
  it('uses the shared standalone shell on first tap and clears the default home active state', () => {
    const dom = bootAdminSchedulerMobileShell({ withNavigate: false });
    ...
    doc.querySelector('#mob-sheet-dynamic-nav [data-nav-target="admin-tools"]')
      ?.dispatchEvent(new dom.window.MouseEvent('click', { bubbles: true }));

    expect(dom.window.resolvePortalRouteUrl).toHaveBeenCalledWith('admin-tools', 'admin');
    expect(dom.window.location.hash).toBe('#admin-admin-tools');
  });
});
