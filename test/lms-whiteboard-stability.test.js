...
describe('lms whiteboard stability guards', () => {
    it('adds circuit breakers and load dedup in workspace runtime', () => {
        ...
        expect(workspace).toContain('const LMS_WHITEBOARD_LOCAL_SYNC_ECHO_MS = 1500;');
        expect(workspace).toContain('const LMS_WHITEBOARD_BACKEND_RELOAD_TTL_MS = 120000;');
    ...
    it('routes async updates through refreshLmsWhiteboardUi instead of full render', () => {
        ...
        expect(runtime).toContain('function repaintLmsWhiteboardWorkspace');
        ...
        expect(runtime).toContain('function finalizeLmsWhiteboardSectionRender');
        ...
        const refreshBlock = runtime.match(/function refreshLmsWhiteboardUi[\s\S]*?(?=\nfunction )/)?.[0] || '';
        const repaintBlock = runtime.match(/function repaintLmsWhiteboardWorkspace[\s\S]*?(?=\nfunction )/)?.[0] || '';
        expect(refreshBlock).toContain('repaintLmsWhiteboardWorkspace(canonicalKey)');
        ...
        const finalizeBlock = runtime.match(/function finalizeLmsWhiteboardSectionRender[\s\S]*?(?=\nfunction )/)?.[0] || '';
        const viewportBlock = runtime.match(/function syncLmsWhiteboardViewportAfterRender[\s\S]*?(?=\nfunction )/)?.[0] || '';
        expect(renderBlock).toContain('deferPaintForLoad');
        expect(renderBlock).toContain('finalizeLmsWhiteboardSectionRender(canonicalKey, { fitOnce: true })');
    ...
    it('invalidates whiteboard tab cache after sync like live quiz', () => {
        ...
        expect(classroom).toContain('window.invalidateLmsWhiteboardTabCache = invalidateLmsWhiteboardTabCache');
    });
});