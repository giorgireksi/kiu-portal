...
describe('global interaction performance guardrails', () => {
  ...
  it('records lightweight startup marks across primer, shell, and route phases', () => {
    ...
    expect(primer).toContain('collapsed = false;');
  ...
  it('locks particle render to steady 30fps pacing', () => {
    ...
    const burstRuntime = readSource('assets/js/shared/lux-button-burst.js');
    const burstCss = readSource('assets/css/lux-button-burst.css');
    expect(burstRuntime).toContain('collectPerimeterPoints(rect)');
    expect(burstRuntime).toContain("kinds = ['dot', 'spark', 'streak']");
    expect(burstRuntime).toContain('_lastBurstAt');
    expect(burstRuntime).toContain('current - lastAt < BURST_INTERVAL_MS');
    expect(burstRuntime).not.toContain('lux-chip-burst-particle--ring');
    expect(burstRuntime).toContain('if (particle._done) return');
    expect(burstRuntime).toContain('window.spawnLuxChipBurstParticles = spawnLuxChipBurstParticles');
    expect(burstCss).toContain('lux-chip-burst-particle--spark');
    expect(burstCss).toContain('lux-chip-burst-particle--streak');
    expect(burstCss).not.toContain('lux-chip-burst-particle--ring');
    ...
    expect(burstRuntime).toContain('.lux-bg-gallery-tile');
    expect(burstRuntime).toContain('#lux-bg-gallery-upload');
