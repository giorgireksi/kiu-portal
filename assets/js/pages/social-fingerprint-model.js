...
(function initSocialFingerprintModel() {
    ...
    function buildPortfolioFingerprint(runtime) {
        ...
        const resumeKey = text(doc?.resume?.storageKey || doc?.resume?.dataUrl || doc?.resume?.name || '');
        return [
            text(doc?.status || ''),
    ...
    function buildSocialRenderSignature(activePanel, runtime) {
        ...
        return [
            ...
            text(ui.eventsStudentFilter || ''),
            text(ui.eventsComposerSection || ''),
            text(ui.activeScopeType || ''),
            text(ui.activeScopeId || ''),
            text(ui.eventScope || ''),
            text(ui.eventEditorSearch || ''),
            text(ui.eventEditorFaculty || 'all'),
            (Array.isArray(ui.eventEditorSelectedIds) ? ui.eventEditorSelectedIds.map((item) => text(item)).filter(Boolean).join(',') : ''),
            ...
            String(Number(ui.researchPdfPage || 1) || 1),
            String(Number(ui.researchPdfZoom || 1) || 1),
            text(ui.profileTab || ''),
