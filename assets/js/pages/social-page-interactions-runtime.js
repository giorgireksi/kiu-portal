...
function finalizeSetPanel(nextPanel, panelChanged, drawerChanged, workspaceNavChanged = false, options = {}) {
    ...
    // Caller will paint once (e.g. panel-messages) — avoid replacing the full UI unnecessarily.
...
function renderSocialPageNow(reason = 'manual') {
    // Social render orchestration is the single owner of panel mounts.
    ...
    const renderCallback = () => {
        ...
        if (renderPlan.center) {
            ...
            // Social center content now mounts directly without a startup transition
            // staging, replay, or a delayed shell reveal.
            revealShell();
        }
    ...
    if (fastPath || /-module$/.test(reason)) {
        renderDebounceTimer = 0;
        renderCallback();
        return;
    }
    renderDebounceTimer = setTimeout(renderCallback, 80);
}
